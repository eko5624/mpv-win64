...
# Delete assets
asset_id=($($DIR/bin/curl -u $GITHUB_ACTOR:$GH_TOKEN $CURL_RETRIES \
...
for id in "${asset_id[@]}"; do
  $DIR/bin/curl -u $GITHUB_ACTOR:$GH_TOKEN $CURL_RETRIES \
    -X DELETE \
    -H "Accept: application/vnd.github.v3+json" \
...
release_id=$($DIR/bin/curl -u $GITHUB_ACTOR:$GH_TOKEN $CURL_RETRIES \
  -H "Accept: application/vnd.github.v3+json" \
